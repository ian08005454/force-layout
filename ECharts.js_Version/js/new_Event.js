...
function keyword_search(e){
    ...
    function data_filter() {
        ...
        option.series[0].links = option.series[0].categories.filter(category => {
            ...
            if (category.show == true) { //這個show是對應到上方類別切換(不是搜尋後的結果)
        ...
        //因為同一個點可能會出現在source&target，過濾出來會有兩個name，因此需去除
...
Chart.on('legendselectchanged', (category_select) => {
    ...
    option.series[0].categories = data.all_category.filter(category => {
        // if the statement is true, reverse the category show boolean
        ...
        if (keyword.length == 0) {
            ...
            if($('#max_level').val() === '1' ){
                ...
                console.log(test_union_collect);
                return test_union_collect.includes(category.target) || test_union_collect.includes(category.source);
            }
        }
    });
    option.series[0].links = option.series[0].categories.filter(category => {
...
function max_Level(ui_value) {    
    ...
    function keyword_length_verify_pass() {
        ...
        // console.log(union_collect); //node
        ...
        test_union_collect = Array.from(new Set([...union_collect, ...inner_union_collect]));
        console.log(test_union_collect);

        option.series[0].nodes = data.nodes.filter(node => {
            return test_union_collect.includes(node.name);
        });
