//ok 這個and的搜尋是由分支較少的那個點向外搜尋 
// 另外有一個是兩個點同時搜尋 名為new_event copy mouse ver2
// variable
...
function keyword_search(e) {
    ...
    function search_more_and() {//搜尋兩個 and
        ...
        for (let i = 1; i < 4; i++) {//4是自訂的數量
            ...
            minus1 = $(category_collect).not(db_firstkey_search).toArray();
        };
    ...
    function search_more_pass() {//搜尋兩個AND
        $(".max_level").hide();
        $(".common_show_value").hide();
        $(".word_strength").hide();

        if (db_firstkey_search.includes(keyword_search_name)) {
            alert(`Error, same keyword can not search more than twice`);
            return;
        }
    ...
    function search_double() {//找到第二個人並顯示在畫面上
        ...
        //////////////////////////////////////////////////////////////////////////
        function find_main_arr(main_arr, union_collect, srh_name) {
