...
function keyword_search(e) {
    ...
    function search_double() {//找到第二個人並顯示在畫面上
        ...
        last_recurve_arr = firstkey_search;//要search 的根 第一個搜尋的name type:arr
        find_main_arr(last_recurve_arr, main_arr, union_collect);
        /////////////////////////////////////////////////////////////////////////////////////

        function find_main_arr(last_recurve_arr, main_arr, union_collect) {
        console.log("start find main fun");
            for (let key = 0, len = last_recurve_arr.length; key < len; key++) {//尋找與A連的點-1-
                main_arr.push(last_recurve_arr[key]);//錢臻 確定有連結到的點
                // console.log(main_arr);//第一個搜尋的name
            ...
            if (union_collect.includes(keyword_search_name_s[1])) {//有
                ...
                // save_obj.push(option.series[0].categories);
                ...
                for (let i = 0; i < temp_minus.length; i++) {
                    temp_name.length = 0;
                    temp_name= temp_minus[i];//要是arr形式
                    ...
                    // console.log("option.series[0].categories錯了");
                    ...
                    if (temp_collect.length === 2) {//此點除了和原本的點連結還沒有其他點
                        ...
                        i = i - 1;
                        console.log("failtemp_collect" + temp_collect);
                        console.log("fail-temp_minus" + temp_minus);
                        console.log("fail-temp_name" + temp_name);
    ...
    //     console.log(option.series[0].categories);
    //     //////////////////type1////////////////////////////
    //     console.log("union_collect" + union_collect);//arr

    //     union_collect = Array.from(new Set(union_collect));//["錢臻","錢俊", "方受疇", "錢楷", "吳璥"]
    //     console.log("union_collect" + union_collect);//arr
    //     //第一層級找到第二個要搜尋的東西
    //     if (union_collect.includes(keyword_search_name_s[1])) {//如果所連結的點已經有連線的話
    //         option.series[0].categories = data.category.filter(category => {
    //             if (firstkey_search.includes(category.target)) {
    //                 if (keyword_search_name_s[1].includes(category.source)) {
    //                     return category;
    //                 }
    //             } else if (firstkey_search.includes(category.source)) {
    //                 if (keyword_search_name_s[1].includes(category.target)) {
    //                     return category;
    //                 }
    //             }
    ...
    //     } else {
    //         var temp_collect;//收集存在["錢俊", "方受疇", "錢楷", "吳璥"]個別的點
    //         var temp_minus = $(union_collect).not(firstkey_search).toArray();//第一輪排除錢臻
    //         console.log(temp_minus);// ["錢俊", "方受疇", "錢楷", "吳璥"]

    //         var temp_name = [];//i = 0 錢俊
    //         for (let i = 0; i < temp_minus.length; i++) {
    //             temp_name.length = 0;
    //             temp_name.push(temp_minus[i]);
    //             temp_collect = [];
    //             console.log(temp_minus);
    //             console.log("MEEE");
    //             option.series[0].categories = data.category.filter(category => {
    //                 // console.log(temp_minus[i]);
    //                 //判斷arr[i]有沒有其他相連的點
    ...
    //                 i = i - 1;
    //                 console.log("failtemp_collect" + temp_collect);
    //                 console.log("fail-temp_minus" + temp_minus);//剩下的繼續跑回圈
    //                 console.log("fail-temp_name" + temp_name);//此次跑的

    //             } else {
    //                 temp_collect = $(temp_collect).not(firstkey_search).toArray();
    //                 temp_collect = $(temp_collect).not(temp_name).toArray();
    //                 console.log("successtemp_collect" + temp_collect);

    //                 console.log("success-temp_minus" + temp_minus);
    //                 console.log("successtemp_name" + temp_name);
